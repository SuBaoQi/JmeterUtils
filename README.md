...
# 主要功能
* getJAByField(String fieldName, JSONObject jsObject, JSONArray resultJa) 从JSON数据中取出指定节点的数据。
    * 参数fieldName为指定的节点名称
...
* listCompare(JSONArray resultJa, JSONArray db, StringBuffer errorMessage, String fieldName,ArrayList<String> ignoreList) 指定节点的数据和该节点对应数据库中的数据进行字段值的比较
    * 参数resultJa为通过getJAByField()方法组装得到的节点值
    * 参数db为查询出来的数据库值
    * 参数errorMessage为存放错误信息的容器
    * 参数fieldName为指定的节点名称
    * 参数ignoreList为存放不需要比较的字段名称
* dealDbData(JSONArray JA) 对于数据库中字段名称有下划线的,将下划线进行去除
    * 参数resultJa为通过getJAByField()方法组装得到的节点值
#使用说明
* 准备两个测试数据
    * 接口的返回或者请求报文
